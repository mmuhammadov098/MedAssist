...
<style>
  ...
  * { box-sizing:border-box; margin:0; padding:0; }
  ...
  .btn-go { padding:13px 18px; background:linear-gradient(135deg,var(--blue),var(--blue2)); color:white; border:none; border-radius:12px; font-size:20px; cursor:pointer; }
  ...
  .section-title { font-size:13px; font-weight:900; text-transform:uppercase; letter-spacing:.5px; margin-bottom:6px; display:flex; align-items:center; gap:6px; }
  .section-card.tarkibi .section-title  { color:#1d4ed8; }
  .section-card.dozasi .section-title   { color:#15803d; }
  .section-card.foydasi .section-title  { color:#a16207; }.section-card.zarari .section-title   { color:#be123c; }
  .section-body { font-size:14px; line-height:1.75; color:var(--text); }
  .loading-box { background:#f8faff; border:1.5px solid var(--border); border-radius:14px; padding:24px; text-align:center; color:var(--muted); font-size:14px; }
...
<div class="card">
  <div class="header">
    <h1>MedAssist Pro</h1>
    <p>AI yordamida dori malumotlari - 3 tilda</p>
  </div>
  <div class="body">
    <div class="search-row">
      <input type="text" id="drug-input" placeholder="Dori nomi... (masalan: Paracetamol)" onkeydown="if(event.key==='Enter') ask()">
      <button class="btn-go" id="btn-search" onclick="ask()">Search</button>
    </div>
    <div class="lang-group">
      <button class="btn-lang uz active" id="btn-uz" onclick="setL('uz')">UZBEK</button><button class="btn-lang ru" id="btn-ru" onclick="setL('ru')">RUSSIAN</button>
    ...
    <div class="warning">
      DIQQAT: Malumotlar AI tomonidan berilgan. Dorini ichishdan oldin albatta shifokor bilan maslahatlashing!
    </div>
    <div class="alarm-box">
      <h4>Dori ichish eslatmasi</h4>
      <div class="alarm-row">
        <input type="time" id="alarm-time">
        <button class="btn-save" onclick="addAlarm()">Qoshish</button>
      </div>
      <div class="daily-row">
        <label class="toggle">
...
function setL(lang) {
  ...
  for (var i = 0; i < langs.length; i++) {
    var btn = document.getElementById('btn-' + langs[i]);
    if (langs[i] === lang) {
...
function ask() {
  var name = document.getElementById('drug-input').value;
  name = name.trim();
  if (!name) { showToast('Dori nomini kiriting'); return; }
  var box = document.getElementById('result-box');
  var btn = document.getElementById('btn-search');
  btn.disabled = true;
  box.innerHTML = '<div class="loading-box"><span class="spinner"></span> Qidirilmoqda...</div>';
  var xhr = new XMLHttpRequest();
  xhr.open('POST', '/get_drug', true);
  ...
  xhr.onload = function() {
    ...
    try {
      ...
      if (data.error) {
        box.innerHTML = '<div class="placeholder">Xato: ' + data.error + '</div>';
        return;
      }
      var html = '';
      if (data.tarkibi) {
        html += '<div class="section-card tarkibi"><div class="section-title">Tarkibi</div><div class="section-body">' + data.tarkibi + '</div></div>';
      }
      if (data.dozasi) {
        html += '<div class="section-card dozasi"><div class="section-title">Dozasi va Qollanilishi</div><div class="section-body">' + data.dozasi + '</div></div>';
      }
      if (data.foydasi) {
        html += '<div class="section-card foydasi"><div class="section-title">Foydasi</div><div class="section-body">' + data.foydasi + '</div></div>';
      }
      if (data.zarari) {
        html += '<div class="section-card zarari"><div class="section-title">Zarari</div><div class="section-body">' + data.zarari + '</div></div>';
      }
      if (!html) { html = '<div class="placeholder">Malumot topilmadi</div>'; }
      box.innerHTML = html;
    } catch(e) {
      box.innerHTML = '<div class="placeholder">Xato: ' + e.message + '</div>';
    }
  };
  xhr.onerror = function() {
    btn.disabled = false;
    box.innerHTML = '<div class="placeholder">Ulanish xatosi</div>';
  };
  xhr.send(JSON.stringify({name: name, lang: currentLang}));
}

function getAlarms() {
  try { return JSON.parse(localStorage.getItem('med_alarms') || '[]'); } catch(e) { return []; }
}
function saveAlarms(a) { localStorage.setItem('med_alarms', JSON.stringify(a)); }

function addAlarm() {
  var t = document.getElementById('alarm-time').value;
  if (!t) { showToast('Vaqtni tanlang'); return; }
  var daily = document.getElementById('daily-toggle').checked;
  var alarms = getAlarms();
  for (var i = 0; i < alarms.length; i++) {
    if (alarms[i].time === t) { showToast('Bu vaqt allaqachon bor'); return; }}
  alarms.push({time: t, daily: daily});
  saveAlarms(alarms);
  renderAlarms();
  document.getElementById('alarm-time').value = '';
  showToast('Eslatma saqlandi: ' + t);
...
function deleteAlarm(time) {
  ...
  var newAlarms = [];
  for (var i = 0; i < alarms.length; i++) {
    if (alarms[i].time !== time) { newAlarms.push(alarms[i]); }
  }
  saveAlarms(newAlarms);
  renderAlarms();
  showToast('Ochirildi: ' + time);
}
...
function renderAlarms() {
  ...
  if (!alarms.length) {
    list.innerHTML = '<p class="no-alarm">Hozircha eslatma yoq</p>';
    return;
  }
  var html = '';
  for (var i = 0; i < alarms.length; i++) {
    ...
    html += '<span class="atime">' + a.time + '</span>';
    html += '<span class="badge ' + (a.daily ? 'daily' : '') + '">' + (a.daily ? 'Har kuni' : 'Bir marta') + '</span>';
    html += '<button class="alarm-del" onclick="deleteAlarm(\'' + a.time + '\')">X</button>';
    html += '</div>';
  }
  list.innerHTML = html;
}
...
function getFired() {
  try { return JSON.parse(localStorage.getItem('med_fired') || '{}'); } catch(e) { return {}; }
}
function markFired(t) {
  ...
  if (!fd[td]) { fd[td] = []; }
  var found = false;
  for (var i = 0; i < fd[td].length; i++) {
    if (fd[td][i] === t) { found = true; break; }
  }
  if (!found) { fd[td].push(t); }
...
setInterval(function() {
  ...
  for (var i = 0; i < alarms.length; i++) {
    ...
    if (hasFiredToday(a.time)) { continue; }
    markFired(a.time);
    alert('VAQT BOLDI! DORINGIZNI ICHING! Vaqt: ' + cur);
    if (Notification.permission === 'granted') {
      new Notification('MedAssist Pro', {body: cur + ' - Dori ichish vaqti keldi!'});
    }
    if (!a.daily) {
      deleteAlarm(a.time);
    }
...
def get_drug():
    try:
        ...
        logger.info("Muvaffaqiyatli: %s", drug_name)
